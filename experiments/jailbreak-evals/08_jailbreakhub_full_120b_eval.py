#!/usr/bin/env python3
"""
JailbreakHub FULL Evaluation - GPT-OSS Safeguard 120B Model
-----------------------------------------------------------
Full dataset evaluation with MEDIUM reasoning effort (normal).
Tests 120B model on complete balanced dataset for comprehensive baseline.

Dataset: JailbreakHub (walledai) - 1,405 real jailbreaks, 13,735 benign
Sample: ~5900 prompts (balanced: 1405 jailbreaks + 4495 benign)
Paper: "Do Anything Now": Characterizing and Evaluating In-The-Wild Jailbreak Prompts

This establishes comprehensive 120B baseline with medium reasoning effort.
...
Runtime: ~30-40 hours for full dataset
"""
    ...
    def __init__(self):
        try:
            self.detector = SafeguardDetector(
                model="gpt-oss-safeguard:120b",  # 120B model for comprehensive baseline
...
class JailbreakHubEvaluator:
    ...
    def __init__(self, sample_size=5900):
...
def print_results(results):
    ...
    print("\n" + "=" * 100)
    print("JAILBREAKHUB FULL EVALUATION RESULTS - GPT-OSS Safeguard 120B (Medium Reasoning)")
    print("=" * 100)
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Full evaluation on JailbreakHub dataset')
    parser.add_argument('--sample-size', type=int, default=5900,
                        help='Number of prompts to evaluate (default: 5900 for full balanced)')
    args = parser.parse_args()

    print("=" * 100)
    print("JAILBREAKHUB FULL EVALUATION - GPT-OSS Safeguard 120B")
    print("Testing 120B model with MEDIUM reasoning on complete balanced dataset")
    print("Dataset: Real jailbreak techniques from Reddit/Discord (2022-2023)")
    print("=" * 100)

    est_time_hours = args.sample_size * 18.7 / 3600  # Based on 18.7s median latency
    print(f"\n⚠️  NOTE: This will take approximately {est_time_hours:.1f} HOURS to complete")
    print(f"   Sample size: {args.sample_size} prompts")
    print(f"   - Heuristic: ~{args.sample_size * 0.001:.1f} seconds")
    print(f"   - Real-LLM (120B): ~{est_time_hours:.1f} hours (18.7s median per prompt)")
    print(f"   - Layered: Similar (most go through LLM)\n")
    print(f"⏰  Expected completion: ~30-40 hours for full dataset\n")
    ...
    print("\n" + "=" * 100)
    print("✅ EVALUATION COMPLETE - GPT-OSS Safeguard 120B (Full Dataset)")
    print("=" * 100)
    print("\nComprehensive 120B baseline on complete balanced JailbreakHub dataset.")
    ...
    print("Model: OpenAI GPT-OSS Safeguard 120B with MEDIUM reasoning effort")
    print(f"Sample Size: {results['dataset_size']} prompts (full balanced dataset)")
    print("Use Case: Comprehensive performance baseline for 120B model")
